...
class RomModder {
	public $flags = array();
	public $randomGenerator = null;

	function __construct($flags, $initSeed) {
		$this->flags = $flags;
		$this->randomGenerator = new RandomGenerator($initSeed);
	}

	function apply($rom) {
		for($j = 0; $j < $this->flags["StartingMonster"]; $j++){
			//Burn random numbers to make monster choice affect the randomization.
			//TODO: Should all flags do this?
			$this->random();
		}
		
		$this->shuffleMonsterGrowth($rom);
		$this->shuffleMonsterResistances($rom);
		$this->shuffleMonsterSkills($rom);
		$this->shuffleEncounters($rom);
		$this->codePatches($rom);
	...
	function shuffleMonsterGrowth($rom) {
		...
		{
			if ($this->flags["Growth"] == "Redistribute")
			{
				...
				$slot1 = $this->random() % 6; //Named slot1 because C# is throwing a fit if I re-use the same var name in the loop below...
			...
			} elseif ($this->flags["Growth"] == "Shuffle") {
				$options = [];
				for ($j = 0; $j < 6; $j++)
				{
				...
				for ($j = 0; $j < 6; $j++)
				{
					$chosen_offset = $this->random() % count($options);
	...
	function shuffleMonsterResistances($rom) {
		for ($i = 0; $i < $rom->map["monsters"]["count"]; $i++) {
			if ($this->flags["Resistance"] == "Redistribute") {
				$total_resistances = 0;
				for ($j = 0; $j < 27; $j++) {
			...
			} elseif ($this->flags["Resistance"] == "Shuffle") {
				$options = [];
				for ($j = 0; $j < 27; $j++)
				{
	...
	function shuffleMonsterSkills($rom) {
		...
		if ($this->flags["Skills"] == "Random, No Healing") {
		...
		for ($i = 0; $i < $rom->map["monsters"]["count"]; $i++)
		{
			if ($this->flags["Skills"] != "None")
			{
				//Randomize skills!  Pick three of these.
				$skill1 = $this->random() % count($tier_one_skills);
				$skill2 = $this->random() % count($tier_one_skills);
				while ($skill2 == $skill1)
				{
					$skill2 = $this->random() % count($tier_one_skills);
				}
				$skill3 = $this->random() % count($tier_one_skills);
	...
	function shuffleEncounters($rom) {
		...
		for ($i = 0; $i < $rom->map["encounters"]["count"]; $i++)
		{
			...
			if($i == 0 && $this->flags["StartingMonster"] != 0){
				//Allow the starting monster to be selectable
				$monsterid = $this->flags["StartingMonster"];
			}
			elseif($i == $rom->magicValues["HoodSquid Drop Encounter"]){
				...
				$monsterid = $this->random() % 32 + 0x13C;
			} else if ($this->flags["YetiMode"] == "On") {
				$monsterid = $rom->monsterIDsByName["Yeti"];
			} else {
				$monsterid = $rom->allowedMonsterIDs[$this->random() % count($rom->allowedMonsterIDs)];
			}
			...
			if($this->flags["Encounters"] == "Poorly") //Previously "Based On Growth"
			{
				...
				for ($j = 0; $j < 6; $j++)
				{
					...
					if($j == 0 && $is_boss_I_think){
						...
						$scalar = $this->flags["BossScaling"]/100;
						...
						//2018 06 25 - ealm - Adding base stat scaling flags
						$scalar = 1;
						if($is_boss_I_think) $scalar = $this->flags["BossScaling"]/100;
						else $scalar = $this->flags["StatScaling"]/100;
						$new_stat *= $scalar;
			...
			$global_exp_scalar = $this->flags["EXPScaling"]/100;
	...
	function codePatches($rom) {
		...
		for($i = 0; $i < count($Masters); $i++){
			...
			$title = $this->random() % count($Titles);
			$team = $this->random() % count($Teams);
